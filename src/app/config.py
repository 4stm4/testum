...
class Config:
    ...
    NERVUM_CALLBACK_URL: Optional[str] = os.getenv("NERVUM_CALLBACK_URL")  # public URL for webhook registration

    # Nervum SDN — T3 service account tokens
    # Bootstrap admin token — used ONLY to create the testum-sync SA on first run.
    # Never used for regular SDN operations. Set once, then revoke / unset.
    NERVUM_BOOTSTRAP_TOKEN: Optional[str] = os.getenv("NERVUM_BOOTSTRAP_TOKEN")

    # Primary service account token (webhook:read/write, network:read, node:read).
    # Generated by bootstrap_service_account() and stored here or via secret manager.
    NERVUM_TOKEN: Optional[str] = os.getenv("NERVUM_TOKEN")

    # HMAC secret returned once by POST /api/v1/webhooks — store immediately.
