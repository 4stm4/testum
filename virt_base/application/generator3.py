...
class VMConfig:
    def __init__(self, name, memory, vcpu, disk_path, cdrom_iso_path, mac_address, bridge):
        self.name = name
        self.memory = memory
        self.vcpu = vcpu
        self.disk_path = disk_path
        self.cdrom_iso_path = cdrom_iso_path
...
class VMXMLGenerator:
    ...
    def generate_xml(self) -> str:
        ...
        os_type = ET.SubElement(os, 'type', arch='aarch64', machine='virt')
        os_type.text = 'hvm'
        ...
        nvram.text = f'/var/lib/libvirt/qemu/nvram/{self.config.name}_VARS.fd'
